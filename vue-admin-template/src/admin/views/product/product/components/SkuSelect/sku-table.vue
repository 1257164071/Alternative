...
<script>
import Vue from 'vue'
import { diffArary, flatten } from '@admin/utils/sku'
...
export default {
  ...
  methods: {
    ...
    objectSpanMethod({ row, column, rowIndex, columnIndex }) {
      ...
      if (columnIndex > row['skus'].length-1){
        return mergeRow;
      }
      ...
      var nowrow = rowIndex;
      // if (nowrow !== 0){
        // window.console.log([row['skus'][columnIndex]['v'] , this.data[nowrow-1]['skus'][columnIndex]['v']])
      // }
      if (nowrow !== 0 && row['skus'][columnIndex]['v'] === this.data[nowrow-1]['skus'][columnIndex]['v']){
        return {
          rowspan: 0,
          colspan: 1,
        }
      }else {
        while (nowrow < this.data.length && this.data[nowrow]['skus'][columnIndex]['v'] === row['skus'][columnIndex]['v']){
          rowspan++;
          nowrow++;
        }
        return {
